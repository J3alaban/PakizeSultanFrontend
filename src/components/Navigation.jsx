...
import {
  Menu, X, ChevronDown, Info, BookOpen, Utensils,
  LogIn, User, LogOut, ShieldCheck
} from 'lucide-react';
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-2">
            <Link to="/" className="relative px-6 py-2">
              ...
              <span className={`text-sm font-bold transition-colors ${isActivePath('/') ? 'text-[#FF69B4]' : 'text-gray-500 hover:text-[#FF69B4]'}`}>Ana Sayfa</span>
            </Link>
            ...
            >
              <div className="flex items-center gap-1 text-sm font-bold text-gray-500 group-hover:text-[#FF69B4] cursor-pointer transition-colors">
              ...
              <AnimatePresence>
                {isCorporateOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 15 }}
                    className="absolute top-full left-0 mt-2 w-56 bg-white rounded-3xl shadow-xl border border-pink-50 py-3"
                  >
                    {corporateLinks.map((link) => (
                      <Link key={link.path} to={link.path} className="flex items-center gap-3 px-6 py-3 text-sm font-semibold text-gray-600 hover:bg-pink-50 hover:text-[#FF69B4] transition-colors">
            ...
            <Link to="/iletisim" className="px-6 py-2">
              <span className={`text-sm font-bold transition-colors ${isActivePath('/iletisim') ? 'text-[#FF69B4]' : 'text-gray-500 hover:text-[#FF69B4]'}`}>İletişim</span>
            </Link>

            {/* Admin Paneli Linki (Sadece Admin ise) */}
            {user?.role === "ADMIN" && (
              <Link to="/admin" className="px-6 py-2">
                 <span className="text-sm font-bold text-red-500 hover:text-red-600 transition-colors flex items-center gap-1">
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-white overflow-hidden border-t border-gray-50"
          >
            <div className="p-4 space-y-2">
              ...
              <Link to="/" onClick={() => setIsMenuOpen(false)} className="block py-4 px-6 text-gray-600 font-semibold">Ana Sayfa</Link>

              {corporateLinks.map((link) => (
                <Link key={link.path} to={link.path} onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 py-4 px-6 text-gray-600 font-semibold">
                  <link.icon size={18} /> {link.label}
                </Link>
              ))}

              <Link to="/iletisim" onClick={() => setIsMenuOpen(false)} className="block py-4 px-6 text-gray-600 font-semibold">İletişim</Link>

              {user?.role === "ADMIN" && (
                <Link to="/admin" onClick={() => setIsMenuOpen(false)} className="block py-4 px-6 text-red-500 font-bold bg-red-50 rounded-2xl">
                  Admin Paneli
                </Link>
              )}

              {user ? (
                <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-4 text-red-500 font-bold bg-red-50 rounded-2xl italic uppercase mt-4 transition-colors">
                  <LogOut size={20} /> Çıkış Yap
                </button>
              ) : (
                <button onClick={() => { setIsMenuOpen(false); setIsLoginModalOpen(true); }} className="w-full py-4 bg-pink-50 text-[#FF69B4] font-bold rounded-2xl italic uppercase mt-4">
                  Giriş Yap
                </button>
              )}
            </div>
